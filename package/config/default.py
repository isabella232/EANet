...
# ['market1501', 'cuhk03_np_detected_jpg', 'duke']
...
if hasattr(cfg.dataset.test, "query_splits"):
    assert len(cfg.dataset.test.query_splits) == len(
        cfg.dataset.test.names
    ), "If cfg.dataset.test.query_splits is defined, it should be set for each test set."
